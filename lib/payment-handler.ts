...
export async function approvePayment(reference: string, adminUsername: string): Promise<boolean> {
  ...
  try {
    ...
    console.log("Processing order with Wendigg:", {
      target_product_code: order.productCode,
      data: order.userData,
    })
...
// Implementasi API Wendigg yang sebenarnya
async function callWendiggAPI(order: PendingOrder) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/transaction/create`, {
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          server: order.userData.server || undefined,
        },
      }),
    })
